...
class CDC_T(nn.Module):
    ...
    def forward(self, x):
        # TDC(x) = \sum_{p_n\in\mathcal R}[w(p_n)\cdot x(p_0+p_n)] +
        #           \theta(-x(p_0)\cdot\sum_{p_n\in\mathcal R'}w(p_n))
        ...
        if math.fabs(self.theta - 0.0) < 1e-8:
            ...
            if self.conv.weight.shape[2] > 1:
                kernel_diff = self.conv.weight[:, :, 0, :, :].sum(2).sum(2) + \
                              self.conv.weight[:, :, 2, :, :].sum(2).sum(2)  # [in, out]
                kernel_diff = kernel_diff[:, :, None, None, None]  # [in, out, 1, 1, 1]
...
class MultiHeadedSelfAttention_TDC_gra_sharp(nn.Module):
    ...
    def __init__(self, dim, num_heads, dropout, theta):
        ...
        # temporal difference convolution projection
    ...
    def forward(self, x, tau):  # [B, 4*4*40, 128]
        """
        mask : B x T
        * split D(dim) into (H(n_heads), W(width of head)) ; D = H * W
        :param x: B x T x D
        :param tau: 根据指定的 \tau, 计算注意力分数
        :return:
        """
...
class Block_ST_TDC_gra_sharp(nn.Module):
    ...
    def forward(self, x, tau):
        Atten, Score = self.attn(self.norm1(x), tau)
